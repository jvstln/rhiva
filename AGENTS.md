...
- `apps/www` — the app. **Next.js 16.2.9 + React 19**, deployed as a **Cloudflare Worker via OpenNext** (`wrangler.jsonc`, `open-next.config.ts`) — not a Node server. Local dev gets CF bindings from `initOpenNextCloudflareForDev()`. `reactCompiler: true`, `typedRoutes: true`. Paths: `@/*` → `src/*`, `@/public/*` → `public/*`.
